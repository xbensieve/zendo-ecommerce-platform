...
@ActiveProfiles("test")
public class JwtSuspendedAccountAttackTest {
    ...
    @Test
    @DisplayName("Suspending an active account must immediately reject any existing JWT with 401 Unauthorized")
    void suspendedAccount_immediatelyRejectsExistingJwt() {
        String email = "suspend_attack_" + UUID.randomUUID() + "@example.com";
        User user = User.createNew(email, "Victim", "Suspended");
        ...
        // Suspend user in identity repository
        user.suspend();
        userRepository.save(user);

        // Attacker attempts to reuse the same unexpired JWT
